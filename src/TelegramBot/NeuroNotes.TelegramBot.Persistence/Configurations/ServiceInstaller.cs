namespace NeuroNotes.TelegramBot.Persistence.Configurations;

public static class ServiceInstaller
{
    extension(IServiceCollection services)
    {
        public IServiceCollection AddTelegramBotPersistence()
        {
            services.ConfigurePersistenceOptions();

            services.AddDbContext<TelegramBotDbContext>((serviceProvider, options) =>
            {
            ...
            services.AddScoped<DbContext>(serviceProvider => serviceProvider.GetRequiredService<TelegramBotDbContext>());
